...
class NIOHandlerWrap implements NIOHandler {
	protected static final Logger LOGGER = LoggerFactory .getLogger(NIOHandlerWrap.class);
	...
	public void onConnected(Connection con) throws IOException {
		...
		String info = con.getDirection() == Connection.Direction.in ? "remote peer connected to me " + con : " connected to remote peer " + con;
		LOGGER.info(info);
	...
	@SuppressWarnings("unchecked")
	@Override
	public void handle(Connection con, ByteBuffer data, int start, int readeLength) {
