...
/* 所有内存缓存,对缓存的内容不care, 就是一个KV缓存 */
public class CacheService {
    private static final Logger logger = LoggerFactory.getLogger(CacheService.class);

	private final Map<String/*factory name*/, CachePoolFactory> poolFactorys = new HashMap<String, CachePoolFactory>();
	private final Map<String/*pool name*/, CachePool> allPools = new HashMap<String, CachePool>();
	...
	public Map<String, CachePool> getAllCachePools() { return this.allPools; }

	private void init() throws Exception {
		Properties props = new Properties();
		props.load(CacheService.class.getResourceAsStream("/caches.properties"));
	...
	private void createLayeredPool(String cacheName, String type, int size, int expireSeconds) {
		...
		logger.info("create layer cache pool " + cacheName + " of type " + type + " ,default cache size " + size + " ,default expire seconds" + expireSeconds);
	...
	/* 创建缓存工厂 */
	private void createPoolFactory(String factryType/*工厂类型*/, String factryClassName/*工厂类名*/) throws Exception {
